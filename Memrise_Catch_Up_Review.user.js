...
/*
    Unfortunately the script is no longer working - repeated requests no longer extend the interval..
*/
...
if(!isBroken) {
    $(document).ready(function() {
        console.log("catch up review script loaded");
        ...
        $(document).ajaxSuccess(
            function(event, request, settings) {
                var response = request.responseJSON,
                    ...
                    box = validRequest && _.findLast(MEMRISE.garden.boxes._list, {
                ...
                if (validRequest && box.scheduled) {
                    var catchUpCount = parseInt(getValue(settings.data, "catchups"), 10) || 0,
                        ...
                        lastDate = new Date(getValue(settings.data, "lastDate") || box.thinguser.last_date || new Date()),
